...
-- rakamın basamak değeri gözönüne alınarak metin oluşturuluyor
  ...
  when 1 then
         case @basamak % 3 
			...
			when 2 then N'on' 
			when 1 then
			       case when @toplambasamak = 4 and @i = 1 then N'' 
				        else
					    case when @i = @toplambasamak then N'bir' 
						     else
							 case when substring(@sayivarchar, @i-2, 2) = '00' then N'' 
								  else N'bir'
							 end
					    end
		           end
  ...
  when 2 then
         case @basamak % 3
			 when 0 then N'iki yüz' 
	         when 2 then N'yirmi' 
	         when 1 then N'iki' 
         end
  when 3 then
	     case @basamak % 3
			when 0 then N'üç yüz' 
			when 2 then N'otuz' 
			when 1 then N'üç' 
		 end
  when 4 then
         case @basamak % 3
			when 0 then N'dört yüz' 
			when 2 then N'kırk' 
			when 1 then N'dört' 
		 end
  when 5 then
         case @basamak % 3
			when 0 then N'beş yüz' 
			when 2 then N'elli' 
			when 1 then N'beş' 
		 end
  when 6 then
         case @basamak % 3
			when 0 then N'altı yüz' 
			when 2 then N'altmış' 
			when 1 then N'altı' 
		 end
  when 7 then
         case @basamak % 3
			when 0 then N'yedi yüz' 
			when 2 then N'yetmiş' 
			when 1 then N'yedi' 
		 end
  when 8 then
         case @basamak % 3
			when 0 then N'sekiz yüz' 
			when 2 then N'seksen' 
			when 1 then N'sekiz' 
		 end
  when 9 then
         case @basamak % 3
			when 0 then N'dokuz yüz' 
			when 2 then N'doksan' 
			when 1 then N'dokuz' 
		 end
  ...
  case @basamak
  when 4 then
         case when substring(reverse(@sayivarchar), @basamak, 3) = '000' then N'' 
		      else N'bin' 
	     end
  when 7 then
         case when substring(reverse(@sayivarchar), @basamak, 3) = '000' then N'' 
